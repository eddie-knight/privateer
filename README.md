...
| `-n` | Leave your shell config alone; print the `PATH` line instead |
...
The install directory is added to your shell config by default — `~/.bashrc`,
`~/.zshrc`, `~/.config/fish/config.fish`, or `~/.profile`, depending on your
shell. Run interactively you get a confirmation prompt; run unattended it goes
ahead. Pass `-n` to opt out and print the `PATH` line instead.

Either way you must restart your shell afterwards: the installer runs in its own
process and cannot change the `PATH` of the shell that started it.
