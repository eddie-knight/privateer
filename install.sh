...
usage() {
    cat <<EOF
Usage: install.sh [-p install_dir] [-v version] [-y | -n] [-h]

  -p  Install directory (default: $DEFAULT_INSTALL_DIR)
  -v  Release to install, e.g. v0.22.0 (default: $DEFAULT_VERSION)
  -y  Add the install directory to PATH without asking
  -n  Do not modify your shell config; print the PATH line instead
  -h  Show this help

The install directory is added to your shell config by default, after a
confirmation prompt when run interactively. Use -n to opt out.
...
# mode is one of:
#   prompt  ask when interactive, otherwise go ahead (the default)
#   yes     go ahead without asking (-y)
#   no      never touch the shell config (-n)
#
# Editing the config cannot affect the shell that invoked this script, so a
# "restart your shell" line is unavoidable on every path that succeeds.
update_path() {
    ...
    case "$(basename "${SHELL:-}")" in
        ...
        fish)
            ...
            ;;
        *)
            # Unrecognised (or unset) shell. ~/.profile is read by most POSIX
            # login shells, so this beats telling the user to do it themselves.
    ...
    case "$mode" in
        ...
        prompt)
            if is_interactive; then
                reply=""
                # `read` returns non-zero at EOF (Ctrl-D). Treat that like empty
                # input, taking the default, rather than letting `set -e` abort
...
main() {
    local install_dir="$DEFAULT_INSTALL_DIR" path_mode=prompt
    local version="${PVTR_VERSION:-$DEFAULT_VERSION}"

    while getopts "p:v:ynh" opt; do
        case "$opt" in
    ...
    update_path "$install_dir" "$path_mode"
