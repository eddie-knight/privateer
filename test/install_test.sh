...
# The default. A non-interactive run has nobody to ask, so it goes ahead rather
# than printing instructions nothing will read.
test_non_interactive_run_modifies_path() {
    local work_dir; setup
    trap 'rm -rf "$work_dir"' RETURN

    local output
    output="$(run_in_sandbox "$work_dir" 'update_path "$1" prompt' "$work_dir/install" 2>&1)"
    assert_contains "$output" "Added $work_dir/install to PATH"
    assert_contains "$(cat "$work_dir/home/.bashrc")" "export PATH=\"$work_dir/install:\$PATH\""
}

test_opt_out_does_not_modify_path() {
    local work_dir; setup
    trap 'rm -rf "$work_dir"' RETURN

    local output
    output="$(run_in_sandbox "$work_dir" 'update_path "$1" no' "$work_dir/install" 2>&1)"
    assert_contains "$output" "add $work_dir/install to your PATH"
    assert_contains "$output" "export PATH=\"$work_dir/install:\$PATH\""
    [[ ! -f "$work_dir/home/.bashrc" ]] || fail "-n must not edit the shell config"
}

test_opt_out_flag_reaches_update_path() {
    local work_dir; setup
    trap 'rm -rf "$work_dir"' RETURN

    local output
    output="$(run_in_sandbox "$work_dir" 'main -p "$1" -n' "$work_dir/install" 2>&1)"
    assert_contains "$output" "Installed $work_dir/install/pvtr"
    [[ ! -f "$work_dir/home/.bashrc" ]] || fail "-n must not edit the shell config"
}
...
test_path_is_modified_with_consent() {
    ...
    run_in_sandbox "$work_dir" 'update_path "$1" yes' "$work_dir/install" > /dev/null
    assert_contains "$(cat "$work_dir/home/.bashrc")" "export PATH=\"$work_dir/install:\$PATH\""

    local output
    output="$(run_in_sandbox "$work_dir" 'update_path "$1" yes' "$work_dir/install" 2>&1)"
    assert_contains "$output" "already references"
    assert_equals "$(grep -c "$work_dir/install" "$work_dir/home/.bashrc")" "1"
}

# Answering the prompt: empty (Enter) and EOF take the default, which is now yes;
...
test_prompt_accepts_yes() {
    ...
    run_in_sandbox "$work_dir" 'is_interactive() { return 0; }; update_path "$1" prompt' \
        "$work_dir/install" > /dev/null 2>&1
    assert_contains "$(cat "$work_dir/home/.bashrc")" "export PATH=\"$work_dir/install:\$PATH\""
}

test_prompt_defaults_to_yes_on_empty_input() {
    local work_dir; setup
    trap 'rm -rf "$work_dir"' RETURN
...
test_prompt_declines_on_n() {
    ...
    local output
    output="$(run_in_sandbox "$work_dir" 'is_interactive() { return 0; }; update_path "$1" prompt' \
        "$work_dir/install" 2>&1)"
    assert_contains "$output" "add $work_dir/install to your PATH"
    [[ ! -f "$work_dir/home/.bashrc" ]] || fail "must not edit shell config when declined"
}
...
test_eof_at_prompt_takes_default_without_aborting() {
    local work_dir; setup
    trap 'rm -rf "$work_dir"' RETURN

    local output
    output="$(run_in_sandbox "$work_dir" \
        'is_interactive() { return 0; }; update_path "$1" prompt; echo REACHED_END' \
        "$work_dir/install" 2>&1)" \
        || fail "expected EOF at the prompt to take the default, not abort" "$output"
...
# An unrecognised shell used to bail with "add it yourself". ~/.profile is read
# by most POSIX login shells, so it is a better answer than giving up.
test_unrecognized_shell_falls_back_to_profile() {
    local work_dir; setup
    trap 'rm -rf "$work_dir"' RETURN

    SHELL=/usr/bin/ksh
    run_in_sandbox "$work_dir" 'update_path "$1" yes' "$work_dir/install" > /dev/null
    assert_contains "$(cat "$work_dir/home/.profile")" "export PATH=\"$work_dir/install:\$PATH\""
}

test_unset_shell_falls_back_to_profile() {
    local work_dir; setup
    trap 'rm -rf "$work_dir"' RETURN
